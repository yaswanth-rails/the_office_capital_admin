class WorkspaceAmenity < ApplicationRecord
  ...
  validates :amenity_id, uniqueness: {scope: :workspace_id}
  ...
  def track_changes_in_table
    if Current.toc.present? && self.saved_changes.present? 
      row = WorkspaceAmenity.find(self.id)
      version = PaperTrail::Version.where('item_type=? and item_id=? and whodunnit=?','WorkspaceAmenity',self.id,Current.toc.id.to_s).last.object_changes rescue nil
      if version.present?
        version_data = version.gsub("\n"," ").gsub("--- ","")
        message = "Admin #{Current.toc.email} with id '#{Current.toc.id}' updated the following fields of WorkspaceAmenity '#{row.id}'"+' '+version_data
        subject = "Admin  with id '#{Current.toc.id}', role #{Current.toc.role.role} updated WORKSAPCE AMENITY table"
        latest_changes = []
